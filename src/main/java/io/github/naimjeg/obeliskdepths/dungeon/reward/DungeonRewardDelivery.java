...
import io.github.naimjeg.obeliskdepths.equipment.ObeliskEquipmentGenerator;
...
public final class DungeonRewardDelivery {
    ...
    public static List<ItemStack> generate(
        ...
        List<ItemStack> base = GENERATOR.generate(
                createContext(level, instance, reward)
        );
        List<ItemStack> generated = new java.util.ArrayList<>(base.size());
        for (int ordinal = 0; ordinal < base.size(); ordinal++) {
            generated.add(ObeliskEquipmentGenerator.generateRewardStack(
                    base.get(ordinal),
                    reward.rewardSeed(),
                    reward.rewardId().value(),
                    reward.instanceId().value(),
                    ordinal
            ));
        }
    ...
    private static BlockPos findRewardSpawnBase(ServerLevel level, BlockPos pos) {
        ...
        for (Direction direction : Direction.Plane.HORIZONTAL) {
            ...
            if (level.hasChunk(
                    SectionPos.blockToSectionCoord(side.getX()),
                    SectionPos.blockToSectionCoord(side.getZ())
            )
