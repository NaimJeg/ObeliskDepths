...
public final class DungeonSessionPresence {
    ...
    public static boolean isInsideDungeonTerritory(
         ...
         */
        return resolveSite(dungeonLevel, session)
    ...
    private static Optional<DungeonSite> resolveSite(
        ...
        Optional<DungeonSite> site = DungeonManagerSavedData.get(dungeonLevel)
                .requireReservedDungeon(
                        session.instanceId(),
                        session.siteKey()
                ).map(ReservedDungeonAggregate::site);
        if (site.isEmpty()) {
            ObeliskDepths.LOGGER.error(
                    "Dungeon session presence metadata invariant violation: instance={}, site={}, action=deny_territory_presence",
                    session.instanceId(),
                    session.siteKey()
            );
        }
