...
public final class DungeonSavedDataInvariantValidator {
    ...
    private static void validatePortalAndDungeonSessions(
        ...
        Set<DungeonInstanceId> portalInstances = new HashSet<>();
        for (PortalSession session : data.portalSessions().all()) {
            portalInstances.add(session.instanceId());
            ...
            if (session.participants().size() > 1) {
                violations.add("portal session has too many participants session="
                        + session.id()
                        + " count="
            ...
            for (UUID participant : session.participants()) {
                if (!participant.equals(session.opener())) {
                    violations.add("portal session contains non-opener participant session="
                            + session.id()
                            + " opener="
                            + session.opener()
                            + " participant="
        ...
        for (DungeonSession session : data.sessions().all()) {
            ...
            if (portalInstances.contains(session.instanceId())
                    && session.accessPolicy() != SessionAccessPolicy.STARTER_ONLY) {
                violations.add("portal-created dungeon session must be STARTER_ONLY session="
                        + session.id()
                        + " instance="
                        + session.instanceId()
                        + " actual="
