...
public final class DungeonRewardReconciliation {
    ...
    public static void recoverOpenedPhysicalReward(
        ...
        if (chest.isPresent()) {
            ...
            if (!reconcilePhysicalDeliveryPlan(
                    level,
                    data,
                    reward,
                    chest.get(),
                    mainPos
            ) || !reconcileSpawnedPhysicalOrdinals(
                    level,
                    data,
                    reward,
                    chest.get(),
                ...
                ObeliskDepths.LOGGER.warn(
                        "Unable to reconcile opened physical reward delivery: instance={}, reward={}, pos={}, reason={}",
                        instanceId,
                        rewardId,
                        mainPos,
                        reason
            ...
            boolean deliveryComplete = reward.deliveryPlan()
                    .map(RewardDeliveryPlan::complete)
                    .orElseGet(() -> !chest.get().hasPendingRewards());
            if (deliveryComplete) {
    ...
    public static boolean reconcilePhysicalDeliveryPlan(
            ServerLevel level,
        ...
        if (level == null
                ...
                || reward.rewardPos().filter(mainPos::equals).isEmpty()
        ...
        if (!reward.hasDeliveryPlan()) {
            List<ItemStack> pendingStacks = chest.copyContents();
            if (pendingStacks.isEmpty()) {
                return true;
            }
            if (!data.rewards().beginDeliveryPlan(
                    reward,
                    pendingStacks,
        ...
        RewardDeliveryPlan plan = reward.deliveryPlan().orElseThrow();
        int finalOrdinal = plan.finalOrdinal();
        int chestOrdinal = Math.max(chest.nextSprayOrdinal(), plan.firstOrdinal());
        int expectedPendingStacks = finalOrdinal - chestOrdinal;
        if (chestOrdinal > finalOrdinal
                || chest.copyContents().size() != expectedPendingStacks) {
            ObeliskDepths.LOGGER.error(
                    "Physical reward delivery state diverged: instance={}, reward={}, planFirst={}, planNext={}, chestNext={}, plannedStacks={}, pendingStacks={}",
                    reward.instanceId(),
                    reward.rewardId(),
                    plan.firstOrdinal(),
                    plan.nextOrdinal(),
        ...
        while (plan.nextOrdinal() < chestOrdinal) {
            if (!data.rewards().advanceDeliveryPlan(reward)) {
                return false;
            }
            plan = reward.deliveryPlan().orElseThrow();
        }

        if (!chest.reconcileDeliveryProgress(plan)) {
            return false;
        }
    ...
    private static boolean reconcileSpawnedPhysicalOrdinals(
            ServerLevel level,
            DungeonManagerSavedData data,
            DungeonRewardRecord reward,
            ObeliskChestBlockEntity chest,
        ...
        RewardDeliveryPlan plan = reward.deliveryPlan().orElse(null);
        while (plan != null
                && !plan.complete()
                && DungeonRewardDelivery.hasSpawnedOrdinal(
                        level,
                        reward,
                        plan.nextOrdinal(),
                        mainPos
