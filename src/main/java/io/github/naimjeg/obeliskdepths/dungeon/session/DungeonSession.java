...
public final class DungeonSession {
    public static final Codec<DungeonSession> CODEC =
            RecordCodecBuilder.create(instance -> instance.group(
                    ...
                    DungeonCodecs.UUID_CODEC.listOf()
                            ...
                            .forGetter(session -> sortedUuids(session.participants)),
                    DungeonCodecs.UUID_CODEC.listOf()
                            .optionalFieldOf("physical_participants", List.of())
                            .forGetter(session -> sortedUuids(session.physicalParticipants)),
                    DungeonCodecs.UUID_CODEC.listOf()
                            .optionalFieldOf("spawned_entity_ids", List.of())
                            .forGetter(session -> sortedUuids(session.spawnedEntityIds)),
                    Codec.LONG.optionalFieldOf("created_at_game_time", 0L)
                            .forGetter(DungeonSession::createdAtGameTime),
                    // Legacy save key retained for backward compatibility.
                    Codec.LONG.optionalFieldOf("last_starter_inside_game_time", 0L)
                            .forGetter(DungeonSession::lastParticipantInsideGameTime),
    ...
    private long lastParticipantInsideGameTime;
    ...
    public DungeonSession(
        ...
        this.accessPolicy = requireNonNull(accessPolicy, "access policy");
        this.createdAtGameTime = createdAtGameTime;
        this.lastParticipantInsideGameTime = lastParticipantInsideGameTime;
    ...
    public long lastParticipantInsideGameTime() {
        return this.lastParticipantInsideGameTime;
    }
    ...
    public boolean markPortalEntrySucceeded(
        ...
        changed |= markParticipantInside(gameTime);
    ...
    public boolean markParticipantInside(long gameTime) {
        boolean changed = this.lastParticipantInsideGameTime != gameTime;
        this.lastParticipantInsideGameTime = gameTime;
    ...
    /**
     * Reverses an incomplete cleanup.
     *
     * <p>This is the only rollback path that directly assigns
     * {@link #state} and {@link #tributeBonusActive}. It must only be called
     * by the owning persistence transaction after cleanup has changed the
     * session to {@link DungeonSessionState#CLEANED}.
     *
     * @param previousState previous session state captured before cleanup
     * @param previousTributeBonusActive previous tribute-bonus state
     */
    public void restoreAfterAbortedCleanup(
            DungeonSessionState previousState,
            boolean previousTributeBonusActive
    ) {
        DungeonSessionState restoredState = Objects.requireNonNull(
                previousState,
                "previousState"
        );

        if (this.state != DungeonSessionState.CLEANED) {
            throw new IllegalStateException(
                    "restoreAfterAbortedCleanup requires CLEANED state, was "
                            + this.state
            );
        }

        this.state = restoredState;
        this.tributeBonusActive = previousTributeBonusActive;
    }
    ...
    private static List<UUID> sortedUuids(Collection<UUID> values) {
