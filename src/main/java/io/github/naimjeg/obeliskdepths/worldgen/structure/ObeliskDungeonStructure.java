...
import io.github.naimjeg.obeliskdepths.dungeon.preparation.DungeonStructureReferenceCompliance;
import io.github.naimjeg.obeliskdepths.dungeon.preparation.DungeonStructureReferenceEnvelope;
...
import io.github.naimjeg.obeliskdepths.worldgen.structure.generation.*;
 ...
 * - expose projection metadata through LoadedDungeonSiteReader
 */
public final class ObeliskDungeonStructure extends Structure {
    public static final int MAX_SPATIAL_LAYOUT_ATTEMPTS = 8;
    public static final Identifier DEFAULT_THEME_ID =
            Identifier.fromNamespaceAndPath(ObeliskDepths.MOD_ID, "great_swamp");

    /*
     * World-generation switch retained for controlled temporary shutdowns.
     * Automatic dungeon structure generation is currently restored.
     */
    private static final boolean DUNGEON_WORLD_GENERATION_ENABLED = true;
    ...
    protected Optional<GenerationStub> findGenerationPoint(
        ...
        if (!DUNGEON_WORLD_GENERATION_ENABLED) {
        ...
        for (int attemptIndex = 0;
            ...
            DungeonPiecePlan piecePlan = null;

            try {
                ResolvedDungeonLayout resolvedLayout =
                        DungeonGenerationPlanner.resolveLayout(
                        graph,
                        analysis,
                        previewLayoutOrigin,
                        catalog,
                ...
                generationPlan = DungeonGenerationPlanner.fromResolved(resolvedLayout);
                piecePlan = DungeonPiecePlanCompiler.compile(
                        resolvedLayout,
                        context.structureTemplateManager(),
                        traceContext
                );
                DungeonStructureReferenceCompliance initialCompliance =
                        DungeonStructureReferenceCompliance.analyze(
                                chunkPos,
                                piecePlan
                        );
                DungeonStructureReferenceEnvelope.ChunkAlignedBlockOffset offset =
                        initialCompliance.referenceEnvelope()
                                .chunkAlignedOffsetToContain(
                                        initialCompliance.overallPieceChunkBounds()
                                )
                                .orElseThrow(() -> new DungeonLayoutGenerationException(
                                        "Compiled dungeon footprint is wider than vanilla reference envelope: "
                                                + initialCompliance.describeSummary()
                                ));
                if (!offset.isZero()) {
                    generationPlan = generationPlan.translated(
                            offset.x(),
                            0,
                            offset.z()
                    );
                    piecePlan = piecePlan.translated(
                            offset.x(),
                            0,
                            offset.z()
                    );
                ...
                piecePlan = piecePlan.translatedY(terrainSample.verticalOffset());
                BoundingBox translatedBounds = translatedPlan.siteBounds();
                BlockPos startRoomAnchor = translatedPlan.primaryEntryAnchor();
                /*
                 * Vanilla writes structure references by scanning starts only
                 * within ChunkStatus.MAX_STRUCTURE_DISTANCE (8) of the target
                 * chunk. Feature placement later calls StructureStart.placeInChunk
                 * only for referenced starts, so forcing a remote chunk to FULL
                 * cannot repair a piece outside this envelope.
                 */
                ...
                if (!compliance.compliant()) {
                    throw new DungeonLayoutGenerationException(
                            "Compiled dungeon pieces exceed vanilla reference envelope: "
                                    + compliance.describeSummary()
                                    + " outsidePieces="
                                    + compliance.distanceReport().describeOutsidePieces()
                    );
                }
                BoundingBox effectiveStartBounds =
                        this.adjustBoundingBox(
                                compliance.overallPieceBlockBounds()
                        );
                if (!compliance.effectiveStartBoundsCompliant(effectiveStartBounds)) {
                    throw new DungeonLayoutGenerationException(
                            "Terrain-adjusted structure start bounds exceed vanilla reference envelope: "
                                    + compliance.describeSummary()
                                    + " effectiveStartBounds="
                                    + effectiveStartBounds
                                    + " terrainAdaptation="
                ...
                traceTerrainSample(
                        ...
                        generationPlan.rooms().size()
                                + (DungeonCorridorCellResolver
                                .resolve(generationPlan)
                                .isEmpty()
                                ? 1
                                : 2)
                );
                ...
                ObeliskDepths.LOGGER.debug(
                        ...
                        corridorCells,
                        piecePlan.pieces().size(),
                        piecePlan.corridorCount(),
                        compliance.maximumPieceDistance(),
                ...
                DungeonPiecePlan acceptedPlan = piecePlan;
                return Optional.of(new GenerationStub(
                        startRoomAnchor,
                        builder -> DungeonPiecePlanEmitter.emit(
                                builder,
                                acceptedPlan
                        )
                ));
    ...
    private static void tracePlanAttemptSummary(
        ...
        int corridorCells = plan == null
                ? 0
                : DungeonCorridorCellResolver.resolve(plan).size();
        String compiledPieces = piecePlan == null
                ? "<unavailable>"
                : Integer.toString(piecePlan.pieces().size());
        String compiledRoomPieces = piecePlan == null
                ? "<unavailable>"
                : Long.toString(piecePlan.roomCount());
        String compiledCorridorPieces = piecePlan == null
                ? "<unavailable>"
                : Long.toString(piecePlan.corridorCount());
