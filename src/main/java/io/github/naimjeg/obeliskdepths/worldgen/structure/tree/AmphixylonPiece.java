...
public final class AmphixylonPiece extends StructurePiece {
    ...
    private AmphixylonField field;

    public AmphixylonPiece(
            Role role,
            int descriptorIndex,
            AmphixylonSite site,
        ...
        super(
                ModWorldgen.AMPHIXYLON_PIECE.get(),
                0,
                boundingBox
        );
        this.role = role;
    ...
    public AmphixylonPiece(
        ...
        this.site = new AmphixylonSite(
                ...
                tag.getIntOr(TAG_MAX_RADIUS, AmphixylonSite.MIN_RADIUS),
    ...
    public void postProcess(
        ...
        AmphixylonField treeField = field();
        switch (this.role) {
    ...
    private AmphixylonField field() {
        if (this.field == null) {
            this.field = new AmphixylonField(this.site);
        }
        return this.field;
    }
    ...
    private void placeTrunk(
            WorldGenLevel level,
            BoundingBox clipped,
            AmphixylonField treeField
    ) {
        BlockState logY = amphixylonLog(Direction.Axis.Y);
        BlockState wood = ModBlocks.AMPHIXYLON.wood().get().defaultBlockState();
        BlockState air = Blocks.CAVE_AIR.defaultBlockState();
        BlockPos.MutableBlockPos pos = new BlockPos.MutableBlockPos();
        AmphixylonField.TrunkSlice[] slices =
                treeField.trunkSlices(clipped.minY(), clipped.maxY());

        for (int y = clipped.minY(); y <= clipped.maxY(); y++) {
            AmphixylonField.TrunkSlice slice = slices[y - clipped.minY()];
    ...
    private void placeLowerRoot(
        ...
        BlockState wood = ModBlocks.AMPHIXYLON.wood().get().defaultBlockState();
        BlockState rootTangle = ModBlocks.AMPHIXYLON_ROOT_TANGLE.get().defaultBlockState();
    ...
    private void placeBranchVines(
        ...
        BlockState support = ModBlocks.AMPHIXYLON_VINE_BULB.get().defaultBlockState();
        ...
        for (int x = clipped.minX(); x <= clipped.maxX(); x++) {
            for (int z = clipped.minZ(); z <= clipped.maxZ(); z++) {
                AmphixylonField.VineColumn column =
    ...
    private static BlockState amphixylonLog(Direction.Axis axis) {
        BlockState state = ModBlocks.AMPHIXYLON.log().get().defaultBlockState();
    ...
    private static BlockState persistentLeaves() {
        BlockState state = ModBlocks.AMPHIXYLON.leaves().get().defaultBlockState();
    ...
    private static void placeGroundBlend(
        ...
        BlockState current = level.getBlockState(pos);
        if (isProtected(current) || isAmphixylonBlock(current)) {
            return;
        }
    ...
    private static boolean placeVineSupport(
        ...
        if (current.is(support.getBlock())) {
            return true;
        }
        ...
        boolean replaceWithBulb =
                current.isAir()
                        || current.canBeReplaced()
                        || current.is(
                        ModBlocks.AMPHIXYLON
                                .leaves()
                                .get()
                )
                        || current.is(
                        ModBlocks.AMPHIXYLON_ROOT_TANGLE
                                .get()
                );
    ...
    private static boolean isAmphixylonBlock(BlockState state) {
        return state.is(ModBlocks.AMPHIXYLON.log().get())
                || state.is(ModBlocks.AMPHIXYLON.wood().get())
                || state.is(ModBlocks.AMPHIXYLON.leaves().get())
                || state.is(ModBlocks.AMPHIXYLON_ROOT_TANGLE.get())
                || state.is(ModBlocks.AMPHIXYLON_VINE_BULB.get())
