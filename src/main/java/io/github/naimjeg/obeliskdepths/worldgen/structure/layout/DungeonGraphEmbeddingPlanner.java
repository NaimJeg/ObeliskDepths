...
import io.github.naimjeg.obeliskdepths.worldgen.structure.graph.*;
...
public final class DungeonGraphEmbeddingPlanner {
    ...
    private static final long ROOM_SELECTION_SALT = 0x524F4F4D5F504F4FL;    private enum RoutePolicy {         TREE_PLACEMENT,         NON_TREE_EDGE     } 
    ...
    private static void planAvailableNonTreeEdges(
        ...
        for (DungeonGraphEdge edge : graph.edges()) {
            ...
            NonTreeRouteResult result = planNonTreeEdge(
                    edge,
                    source,
                    target,
                    drafts,
    ...
    private static String diagnosticsSummary(
            ...
            Draft source,
            Draft target,
            NonTreeRouteResult result
    ) {
        RouteBounds bounds = routeBounds(Map.of(
                source.id(), source,
                target.id(), target
        ));
        StringBuilder summary = new StringBuilder();
        summary.append(" ");
        summary.append("kind=").append(edge.kind());
        summary.append(" sourceRoom=").append(source.id());
        summary.append(" sourceBox=").append(source.cellBox());
        summary.append(" targetRoom=").append(target.id());
        summary.append(" targetBox=").append(target.cellBox());
        summary.append(" routesAttempted=").append(result.routesAttempted());
        summary.append(" blockedByHardRooms=").append(result.blockedByHardRooms());
        summary.append(" blockedByRoomClearance=").append(result.blockedByRoomClearance());
        summary.append(" blockedByReservedCorridors=").append(result.blockedByReservedCorridors());
        summary.append(" blockedByCorridorClearance=").append(result.blockedByCorridorClearance());
        summary.append(" invalidSourceEndpoint=").append(result.invalidSourceEndpoint());
        summary.append(" invalidTargetEndpoint=").append(result.invalidTargetEndpoint());
        summary.append(" exhaustedStateBudget=").append(result.exhaustedStateBudget());
        summary.append(" openSetExhausted=").append(result.openSetExhausted());
        summary.append(" boundsExhausted=").append(result.boundsExhausted());
        summary.append(" routeBounds=").append(bounds.minX())
                .append(",").append(bounds.minZ())
                .append(",").append(bounds.maxXExclusive())
                .append(",").append(bounds.maxZExclusive());
        return summary.toString();
    }

    private static NonTreeRouteResult planNonTreeEdge(
            DungeonGraphEdge edge,
            Draft source,
            Draft target,
            Map<String, Draft> drafts,
        ...
        int routesAttempted = 0;
        int blockedByHardRooms = 0;
        int blockedByRoomClearance = 0;
        int blockedByReservedCorridors = 0;
        int blockedByCorridorClearance = 0;
        int invalidSourceEndpoint = 0;
        int invalidTargetEndpoint = 0;
        int exhaustedStateBudget = 0;
        int openSetExhausted = 0;
        int boundsExhausted = 0;
            ...
            for (DungeonConnectorSide targetSide : orderedSides(target, source)) {
                for (GridCell start : exteriorCells(source.cellBox(), sourceSide, target)) {
                    for (GridCell goal : exteriorCells(target.cellBox(), targetSide, source)) {
                        routesAttempted++;
                        DiagnosticRouteResult diagnostic = routeGridPath(
                                drafts,
                                reservedCorridors,
                                bounds,
                                edge.sourceNodeId(),
                                ...
                                start,
                                goal,
                                RoutePolicy.NON_TREE_EDGE
                        );
                        List<GridCell> path = diagnostic.path();

                        if (path.isEmpty()) {
                            blockedByHardRooms += diagnostic.blockedByHardRooms();
                            blockedByRoomClearance += diagnostic.blockedByRoomClearance();
                            blockedByReservedCorridors += diagnostic.blockedByReservedCorridors();
                            blockedByCorridorClearance += diagnostic.blockedByCorridorClearance();
                            invalidSourceEndpoint += diagnostic.invalidSourceEndpoint();
                            invalidTargetEndpoint += diagnostic.invalidTargetEndpoint();
                            exhaustedStateBudget += diagnostic.exhaustedStateBudget();
                            openSetExhausted += diagnostic.openSetExhausted();
                            boundsExhausted += diagnostic.boundsExhausted();
                            continue;
                        }
        ...
        return new NonTreeRouteResult(
                Optional.ofNullable(best),
                routesAttempted,
                blockedByHardRooms,
                blockedByRoomClearance,
                blockedByReservedCorridors,
                blockedByCorridorClearance,
                invalidSourceEndpoint,
                invalidTargetEndpoint,
                exhaustedStateBudget,
                openSetExhausted,
                boundsExhausted
        );
    }

    private record NonTreeRouteResult(
            Optional<EdgePlan> plan,
            int routesAttempted,
            int blockedByHardRooms,
            int blockedByRoomClearance,
            int blockedByReservedCorridors,
            int blockedByCorridorClearance,
            int invalidSourceEndpoint,
            int invalidTargetEndpoint,
            int exhaustedStateBudget,
            int openSetExhausted,
            int boundsExhausted
    ) {}
    ...
    private static void openEndpointApron(
            Set<GridCell> blockedCorridors,
            Set<GridCell> reservedCorridors,
            DungeonCellBox room,
            GridCell endpoint
    ) {
        EndpointFace face = endpointFace(room, endpoint);
        if (face == null) {
            throw new IllegalArgumentException(
                    "Route endpoint must be immediately outside exactly one "
                            + "horizontal room face: endpoint="
                            + endpoint
                            + " room="
                            + room
            );
        ...
        int clearance = PHYSICAL_CORRIDOR_CLEARANCE_CELLS;

        /*
         * A vector perpendicular to the outward face direction. This lets the
         * route turn left or right immediately outside the room while crossing
         * an existing corridor's clearance halo.
         */
        int lateralDx = -face.dz();
        int lateralDz = face.dx();

        for (int forward = 0; forward <= clearance + PHYSICAL_CORRIDOR_CLEARANCE_CELLS; forward++) {
            for (int lateral = -clearance;
                 lateral <= clearance;
                 lateral++) {
                ...
                /*
                 * The exact endpoint may be shared by connections attached to the
                 * same room. Other physically occupied corridor cells remain
                 * blocked. Only their expanded clearance halo is locally opened.
                 */
                if (cell.equals(endpoint)
                        || !reservedCorridors.contains(cell)) {
                    blockedCorridors.remove(cell);
                }
            }
        }
    }

    private static EndpointFace endpointFace(
            DungeonCellBox room,
            GridCell endpoint
    ) {
        boolean west = endpoint.x() == room.minX() - 1
                && endpoint.z() >= room.minZ()
                && endpoint.z() < room.maxZExclusive();

        boolean east = endpoint.x() == room.maxXExclusive()
                && endpoint.z() >= room.minZ()
                && endpoint.z() < room.maxZExclusive();

        boolean north = endpoint.z() == room.minZ() - 1
                && endpoint.x() >= room.minX()
                && endpoint.x() < room.maxXExclusive();

        boolean south = endpoint.z() == room.maxZExclusive()
                && endpoint.x() >= room.minX()
                && endpoint.x() < room.maxXExclusive();

        int matchingFaces = (west ? 1 : 0)
                + (east ? 1 : 0)
                + (north ? 1 : 0)
                + (south ? 1 : 0);

        /*
         * The endpoint must be immediately outside exactly one horizontal face.
         * A corner outside two axes, a cell inside the room, or a distant cell
         * cannot define an unambiguous outward direction.
         */
        if (matchingFaces != 1) {
            return null;
        }
    ...
    private static boolean isCorridorLegal(
        ...
        int apronCellCount = Math.min(
                corridorCells.size(),
                PHYSICAL_CORRIDOR_CLEARANCE_CELLS + 1
        );

        for (int index = 0; index < corridorCells.size(); index++) {
            GridCell cell = corridorCells.get(index);

            boolean exactEndpoint = index == 0
                    || index == corridorCells.size() - 1;

            boolean endpointApron = index < apronCellCount
                    || index >= corridorCells.size() - apronCellCount;

            /*
             * A corridor may reuse the exact exterior connector cell of a room.
             * It may not reuse an existing corridor beyond that endpoint.
             */
            if (reservedCorridors.contains(cell) && !exactEndpoint) {
                return false;
            }

            /*
             * Inside the short connector apron, ignore only the expanded clearance
             * halo. Exact occupied corridor cells remain illegal except at the
             * exact first/last path cell handled above.
             */
            if (!endpointApron && reservedExclusion.contains(cell)) {
                return false;
            }

            /*
             * Connector-apron handling must never permit the route to cross an
             * unrelated room or its physical clearance area.
             */
            if (insideAnyRoom(
                    cell,
                    drafts,
    ...
    private static DiagnosticRouteResult routeGridPath(
            Map<String, Draft> drafts,
            Set<GridCell> reservedCorridors,
            RouteBounds bounds,
            String sourceRoomId,
        ...
        Draft source = requireDraft(drafts, sourceRoomId);
        Draft target = requireDraft(drafts, targetRoomId);

        /* Hard room occupancy. */
        Set<GridCell> hardRoomOccupancy = occupiedRoomCells(drafts);

        /* Soft room clearance around unrelated rooms. */
        Set<GridCell> softRoomClearance = unrelatedRoomClearanceCells(
                drafts,
                sourceRoomId,
                targetRoomId
        );
        softRoomClearance.removeAll(hardRoomOccupancy);

        /* Hard reserved corridors. */
        Set<GridCell> hardReservedCorridors = new HashSet<>(reservedCorridors);

        /* Soft corridor clearance. */
        Set<GridCell> softCorridorClearance =
                expandedReservedCorridors(reservedCorridors);
        softCorridorClearance.removeAll(hardReservedCorridors);

        /* Open endpoint aprons in soft sets. */
        openEndpointApron(
                softRoomClearance,
                hardRoomOccupancy,
                source.cellBox(),
        ...
        openEndpointApron(
                softCorridorClearance,
                hardReservedCorridors,
                source.cellBox(),
                start
        );
        openEndpointApron(
        ...
        /* Build final blocked set. */
        Set<GridCell> blocked = new HashSet<>();
        blocked.addAll(hardRoomOccupancy);
        blocked.addAll(softRoomClearance);
        blocked.addAll(hardReservedCorridors);
        blocked.addAll(softCorridorClearance);

        /* Shared endpoints: allow reuse of exterior connector cells. */
        if (policy == RoutePolicy.TREE_PLACEMENT
                || isExteriorConnector(start, source)) {
            blocked.remove(start);
        }
        if (policy == RoutePolicy.TREE_PLACEMENT
                || isExteriorConnector(goal, target)) {
            blocked.remove(goal);
        }
        ...
        if (!bounds.contains(start)
                || hardRoomOccupancy.contains(start)) {
            invalidSourceEndpoint = 1;
            return emptyDiagnostic(
                    blockedByHardRooms, blockedByRoomClearance,
                    blockedByReservedCorridors, blockedByCorridorClearance,
                    invalidSourceEndpoint, invalidTargetEndpoint,
                    exhaustedStateBudget, openSetExhausted, boundsExhausted
            );
        }
        if (!bounds.contains(goal)
                || hardRoomOccupancy.contains(goal)) {
            invalidTargetEndpoint = 1;
            return emptyDiagnostic(
                    blockedByHardRooms, blockedByRoomClearance,
                    blockedByReservedCorridors, blockedByCorridorClearance,
                    invalidSourceEndpoint, invalidTargetEndpoint,
        ...
        PriorityQueue<SearchNode> open =
                new PriorityQueue<>(searchNodeOrder());
        ...
        while (!open.isEmpty()
            ...
            if (current.cell().equals(goal)) {
                return new DiagnosticRouteResult(
            ...
            for (Step step : orderedSteps(current.cell(), goal)) {
                ...
                if (blocked.contains(nextCell)) {
                    if (hardRoomOccupancy.contains(nextCell)) {
                        blockedByHardRooms = 1;
                    } else if (softRoomClearance.contains(nextCell)) {
                        blockedByRoomClearance = 1;
                    } else if (hardReservedCorridors.contains(nextCell)) {
                        blockedByReservedCorridors = 1;
                    } else if (softCorridorClearance.contains(nextCell)) {
                        blockedByCorridorClearance = 1;
                    }
                    continue;
                }
                ...
                open.add(new SearchNode(
                        ...
                        tentativeCost,
                        tentativeCost
                                + manhattan(nextCell, goal)
                                * CORRIDOR_LENGTH_WEIGHT,
                        sequence++
                ));
            }
        }
    ...
    private static DiagnosticRouteResult emptyDiagnostic(
            int blockedByHardRooms,
            int blockedByRoomClearance,
            int blockedByReservedCorridors,
            int blockedByCorridorClearance,
    ...
    private static boolean isExteriorConnector(GridCell cell, Draft room) {
        DungeonCellBox box = room.cellBox();
        return (cell.x() >= box.minX() - 1
    ...
    private record DiagnosticRouteResult(
            List<GridCell> path,
            int blockedByHardRooms,
            int blockedByRoomClearance,
            int blockedByReservedCorridors,
    ...
    private static Set<GridCell> occupiedRoomCells(Map<String, Draft> drafts) {
    ...
    private static Set<GridCell> unrelatedRoomClearanceCells(
            Map<String, Draft> drafts,
            String sourceRoomId,
            String targetRoomId
    ) {
        Set<GridCell> clearance = new HashSet<>();
        for (Draft draft : drafts.values()) {
            ...
            DungeonCellBox box = draft.cellBox()
                    .expanded(PHYSICAL_CORRIDOR_CLEARANCE_CELLS);
            for (int x = box.minX(); x < box.maxXExclusive(); x++) {
                for (int z = box.minZ(); z < box.maxZExclusive(); z++) {
                    clearance.add(new GridCell(x, z));
                }
            }
        }
        return clearance;
