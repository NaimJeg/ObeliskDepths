...
public final class DungeonLayoutResolver {
    ...
    private static Optional<Map<String, String>> assignPorts(
        ...
        Map<DungeonConnectorSide, String> sidePortAssignments = new LinkedHashMap<>();
        ...
        for (IncidentEdge incident : incidents) {
            ...
            String sharedPortId = sidePortAssignments.get(incident.side());
            if (sharedPortId != null) {
                assigned.put(incident.edgeId(), sharedPortId);
                continue;
            }
            ...
            sidePortAssignments.put(incident.side(), selected.id());
