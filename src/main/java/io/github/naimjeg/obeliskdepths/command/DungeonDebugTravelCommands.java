...
final class DungeonDebugTravelCommands {
    ...
    private static int devStartNearest(
        ...
        Optional<ResolvedDungeonSite> resolved = WorldgenDungeonSiteProvisioner.findOrGenerateReservableSite(
                level.get(),
                origin,
                DungeonManagerSavedData.get(level.get())
        );
        if (resolved.isEmpty() || !resolved.get().authoritative()) {
            ...
            DungeonDebugCommandUtil.failure(
                    source,
                    "dev-start-nearest did not find an authoritative generated site."
            );
            return 0;
        }
        ...
        DungeonManagerSavedData.get(level.get()).instances().addParticipant(
                instance.id(),
        ...
        DungeonSessionLifecycle.getOrCreateDebugSession(
                level.get(),
                instance,
                enteredPlayer.getUUID()
        );
