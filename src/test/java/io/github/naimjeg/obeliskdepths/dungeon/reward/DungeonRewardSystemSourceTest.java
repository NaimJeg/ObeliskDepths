...
public final class DungeonRewardSystemSourceTest {
    ...
    private static void testReturnScrollGenerationDoesNotReadDifficulty() throws IOException {
        ...
        String method = slice(generator, "public static void generateReturnScrolls", "private static ItemStack chooseFallbackWeapon");
    ...
    private static void testTimedPhysicalSprayLifecycle() throws IOException {
        ...
        assertTrue(blockEntity.contains("reconcileDeliveryProgress"), "block entity must synchronize to persisted delivery facts");
        assertTrue(blockEntity.contains("spawnRewardStack("), "block entity must spawn one reward unit per tick interval");
        assertTrue(blockEntity.contains("!alreadySpawned && !DungeonRewardDelivery.spawnRewardStack"),
                "physical spray must check the current reward ordinal before spawning");
        assertTrue(blockEntity.contains("completePhysicalRewardSpray"), "empty queue must complete through reward claim");
        assertTrue(blockEntity.contains("storedReward.get().status() != DungeonRewardStatus.OPENED"),
                "ticker must verify persisted reward is in progress");
        ...
        String physicalStart = slice(claim, "private static DungeonRewardClaimResult startPhysicalRewardSpray", "private static DungeonRewardClaimResult tryClaimVirtualReward");
        assertOrder(physicalStart, "data.rewards().markOpened(reward)", "data.rewards().beginDeliveryPlan(",
                "physical claim must enter OPENED before creating its delivery plan");
        assertOrder(physicalStart, "data.rewards().beginDeliveryPlan(", "chestBlock.setOpened(level, mainPos, true)",
                "physical delivery plan must exist before world mutation begins");
        ...
        assertFalse(claim.contains("isPhysicalRewardSprayInProgress"),
                "obsolete physical spray status facade must be removed");

        String physicalTick = slice(blockEntity, "public static void serverTick", "private static void tickPostSprayCleanup");
        assertOrder(physicalTick, "DungeonRewardDelivery.spawnRewardStack(", "data.rewards().advanceDeliveryPlan(reward)",
                "physical spray records the spawned ordinal in SavedData immediately after spawn");
        assertOrder(physicalTick, "data.rewards().advanceDeliveryPlan(reward)", "chest.reconcileDeliveryProgress(advancedPlan)",
                "persisted delivery progress must advance before the chest removes its stack");

        String reconciliation = read("src/main/java/io/github/naimjeg/obeliskdepths/dungeon/reward/DungeonRewardReconciliation.java");
        assertTrue(reconciliation.contains("reconcileSpawnedPhysicalOrdinals"),
                "physical recovery must reconcile tagged entities by ordinal");
        assertTrue(reconciliation.contains("chest.nextSprayOrdinal()"),
                "physical recovery must reconcile the block entity's durable ordinal");
        String physicalRecovery = slice(reconciliation, "private static boolean reconcileSpawnedPhysicalOrdinals", "public static void reconcileClaimingRewards");
        assertOrder(physicalRecovery, "DungeonRewardDelivery.hasSpawnedOrdinal(", "data.rewards().advanceDeliveryPlan(reward)",
                "an already-spawned physical ordinal must advance the persisted plan without respawning");
    ...
    private static void testOrdinalVirtualDeliveryLifecycle() throws IOException {
        ...
        assertTrue(plan.contains(".optionalFieldOf(\"first_ordinal\", 0)"), "delivery plan must preserve absolute ordinals for legacy physical sprays");
        ...
        assertTrue(plan.contains("nextOrdinal < firstOrdinal"), "delivery plan rejects invalid absolute ordinals");
        ...
        assertOrder(beginPlan, "DungeonRewardRecord stored = this.requireStored(reward);", "stored.beginDeliveryPlan(stacks, firstOrdinal)", "begin plan mutates canonical reward");
        assertOrder(beginPlan, "stored.beginDeliveryPlan(stacks, firstOrdinal)", "this.dirty.run();", "begin plan marks dirty after mutation");
        ...
        assertOrder(claim, "data.rewards().markClaiming(reward)", "data.rewards().beginDeliveryPlan(reward, stacks, 0)", "claim persists CLAIMING before the plan");
        assertOrder(claim, "data.rewards().beginDeliveryPlan(reward, stacks, 0)", "deliverVirtualRewardPlan(level, data, reward, resolvedDeliveryPos)", "claim persists plan before delivery");
