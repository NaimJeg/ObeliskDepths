...
public final class DungeonReservedRuntimeTeardownTransactionTest {
    ...
    public static void main(String[] args) {
        ...
        legacyStarterOnlyPairPassesInvariant();
    }
    ...
    private static void legacyStarterOnlyPairPassesInvariant() {
        DungeonManagerSavedData data =
                new DungeonManagerSavedData();
        DungeonSite site = createSite(
                new DungeonSiteKey(50, 51),
                320,
                336
        );
        DungeonInstanceCreation creation =
        ...
        UUID starter = uuid("legacy-pair-starter");

        data.territories().put(territory);
        data.instances().put(instance);
        data.sites().reserve(site, instance.id(), RESERVED_AT);
        data.roomStates().initializeRoomStates(instance, site);

        PortalSession explicitOpen = new PortalSession(
                new PortalSessionId(uuid("legacy-pair-portal")),
                instance.id(),
                starter,
                OVERWORLD,
                new BlockPos(0, 64, 0),
        ...
        JsonObject legacyPortalJson =
                PortalSession.CODEC.encodeStart(JsonOps.INSTANCE, explicitOpen)
                        .getOrThrow()
                        .getAsJsonObject();
        assertTrue(
                legacyPortalJson.remove("access_policy") != null,
                "legacy portal fixture starts with a serialized policy"
        );
        PortalSession legacyPortal =
                PortalSession.CODEC.parse(JsonOps.INSTANCE, legacyPortalJson)
                        .getOrThrow();
        assertEquals(
                SessionAccessPolicy.STARTER_ONLY,
                legacyPortal.accessPolicy(),
                "legacy portal without access_policy decodes as STARTER_ONLY"
        );

        DungeonSession dungeon = DungeonSession.createActive(
                instance,
                starter,
                SessionAccessPolicy.STARTER_ONLY,
                false,
        ...
        data.portalSessions().add(legacyPortal);
        data.sessions().add(dungeon);

        DungeonSavedDataInvariantValidator.validateOrThrow(data);
    }
